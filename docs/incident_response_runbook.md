...
2. consistency.xrp_free_vs_net_inventory_diffの絶対値が大きい
   目安: 1レベルあたりの取引量(amount_per_level_xrp、config.py参照)の
   2倍を超える乖離がある場合はエスカレーション。
   (これは「帳簿バグ再発」の兆候である可能性が高い。過去に-48XRP・
   -94.5XRPのような大きな乖離が発生したことがあり、これは実際には
   reconcileの取りこぼしや二重計上が原因だった。軽微な差異
   [1XRP未満程度]は正常な運用のズレとして許容範囲内。)
